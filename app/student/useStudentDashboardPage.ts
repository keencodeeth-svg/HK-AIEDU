...
import { CATEGORY_META, ENTRY_ITEMS, STUDENT_DASHBOARD_GUIDE_KEY } from "./utils";
...
export function useStudentDashboardPage() {
  ...
  const loadDashboard = useCallback(
    ...
    [clearDashboardState, handleAuthRequired, loadJoinRequests, loadRadarSnapshot, loadSchedule, loadTodayTasks]
  );
  ...
  }, [handleAuthRequired, loadSchedule]);
  ...
  const handleJoinClass = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        setJoinMessage({ text: getStudentDashboardJoinRequestMessage(nextError, "加入失败"), tone: "error" });
      }
    },
    [handleAuthRequired, joinCode, loadJoinRequests]
  );
  ...
  }, [handleAuthRequired, loadRadarSnapshot, loadTodayTasks]);
